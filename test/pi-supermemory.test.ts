...
class FakeSupermemoryClient implements SupermemoryClient {
  ...
  results: SupermemorySearchResult[] = [{ id: "mem_1", content: "Use ramiro-dev-memory for dev-agent memory.", score: 0.91 }];
...
test("search tool queries Supermemory with bounded limit", async () => {
  ...
  assert.match(JSON.stringify(result.details), /ramiro-dev-memory/);
});

test("save tool writes into Supermemory with source metadata", async () => {
  const client = new FakeSupermemoryClient();
  const harness = createHarness();
  createSupermemoryExtension({ client, containerTag: "ramiro-dev-memory", clock: () => 1 }).register(harness.pi);
  ...
  assert.equal(client.saves[0]?.metadata?.container_tag, "ramiro-dev-memory");
});

test("context hook injects recall results before existing messages", async () => {
  const client = new FakeSupermemoryClient();
  const harness = createHarness();
  createSupermemoryExtension({ client, clock: () => 123 }).register(harness.pi);
  ...
  assert.match(result.messages[0]?.content ?? "", /ramiro-dev-memory/);
...
test("status tool reports missing API key without throwing", async () => {
  const harness = createHarness();
  createSupermemoryExtension({ apiKey: "", clock: () => 1 }).register(harness.pi);
